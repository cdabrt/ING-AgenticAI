...
![image info](./documentation/Architecture.drawio.png)
...
### Global overview of the End-to-End Pipeline

This pipeline initiates when the Business Analyst provides regulatory PDF(s). These documents enter the Ingestion system to be processed, chunked, and stored as an embedded vector index (FAISS). The LangGraph Agent Orchestration then takes over, where the regulatory discovery strategist (Agent 1) queries both the local vector store and the web (via MCP Server tools) to gather comprehensive context. Finally, the regulatory implementation architect (Agent 2) aggregates all document chunks and web context to generate a consolidated Requirement Bundle. This structured output, containing the derived business and data requirements, is then returned directly to the Business Analyst for review and implementation.  

The agent orchestration includes a critical feedback mechanism to ensure the quality of external information. If the Context Assessor determines that additional context is needed (based on the initial retrieval results), it uses the web_search tool to gather information from the open web. The retrieved Web results are then passed back through the system where they are validated by Agent 1 (regulatory discovery strategist). This step ensures that the newly acquired external data is assessed for relevance and quality before being added to the overall context bundle, guaranteeing that the final requirements generated by Agent 2 are based on verified, aggregated information.  
