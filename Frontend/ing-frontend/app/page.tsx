...
import { RequirementBundle, RequirementItem, PDFItem } from "@/lib/types";
import { AlertCircleIcon, Search, Sparkles, Upload, FileText } from "lucide-react";
...
import PDFList from "@/components/custom/PDFList";
...
export default function Home() {
  ...
  const [selectedList, setSelectedList] = useState<number>(0); // 0 - RequirementBundles, 1 - BusinessRequirements, 2 - DataRequirements, 3 - PDFList
  ...
  function setSelectedListWithAnimation(newValue: number) {
    if (newValue === selectedList) return;
  ...
  function getAllPdfs(): Promise<void> {
    setError(null);
    return axios.get(routes.get_all_pdfs).catch((error) => {
      setError(error.message);
    }).then((response) => {
      if (response && response.data) {
        const pdfs = Array.isArray(response.data) ? response.data : [response.data];
        setPdfList(pdfs);
        if (pdfs.length > 0) {
  ...
  async function loadPdfData(pdfId: number): Promise<void> {
    setError(null);
    try {
  ...
  async function openPdfInModal(pdfId: number): Promise<void> {
    await loadPdfData(pdfId);
    setShowPdfViewer(true);
  }

  async function deletePdf(pdfId: number): Promise<void> {
    setError(null);
    try {
      await axios.delete(routes.delete_pdf(pdfId));
      await getAllPdfs();
    } catch (error: any) {
        ...
        <ResizablePanelGroup direction={"horizontal"}>
          <ResizablePanel defaultSize={1 / 3} className="min-w-1/4">
            <div className="h-full w-full bg-white border-t border-r overflow-hidden">
              {selectedList !== 0 && selectedList !== 3 && (
                <div key={`requirement-detail-${selected?.id}`} className="animate-in fade-in slide-in-from-left-4 duration-300">
                  <RequirementDetailView requirement={selected} onSave={getRequirementBundles} />
                </div>
              )}
              {selectedList === 0 && (
                <div key={`bundle-detail-${selectedBundle?.id}`} className="animate-in fade-in slide-in-from-left-4 duration-300">
                  <BundleDetailView bundle={selectedBundle} />
                </div>
              )}
              {selectedList === 3 && selectedPdf && pdfData && (
                <div key={`pdf-viewer-${selectedPdf.id}`} className="h-full animate-in fade-in slide-in-from-left-4 duration-300">
                  <div className="h-full flex flex-col">
                    <div className="p-4 border-b">
                      <h2 className="text-lg font-bold">{selectedPdf.filename}</h2>
                    </div>
          ...
          <ResizablePanel defaultSize={2 / 3} className="min-w-1/4 flex flex-col">
            <div className="w-full flex flex-row justify-center items-center bg-zinc-200 p-2 space-x-4">
              ...
              <ButtonGroup>
                <Button
                  onClick={async () => {
                    await getAllPdfs();
                    setSelectedListWithAnimation(3);
                  }}
                  variant="outline"
                >
            ...
            <div className="flex-1 w-full bg-white p-4 relative flex flex-col">
              <div className="flex-grow overflow-y-auto">
                ...
                {selectedList > 0 && selectedList < 3 &&
                  <div 
                    key={`requirement-list-${selectedList}`} 
                    className={`flex flex-col space-y-4 transition-opacity duration-200 ${
                      isTransitioning ? 'opacity-0' : 'opacity-100 animate-in fade-in slide-in-from-right-4 duration-300'
                    }`}
                  >
                    ...
                    <RequirementList
                      ...
                      onGoBack={() => setSelectedListWithAnimation(0)}
                    />
                  </div>
                }
                {
                  ...
                  >
                    <PDFList
                      ...
                      onGoBack={() => setSelectedListWithAnimation(0)}
                      onDownload={(pdf) => openPdfInModal(pdf.id)}
                      onDelete={(pdf) => deletePdf(pdf.id)}
                    />
                  </div>
                }
                {
                  ...
                  >
                    <RequirementBundleList
                      ...
                      setSelectedList={setSelectedListWithAnimation}
                    />
                  </div>
                }
              </div>
